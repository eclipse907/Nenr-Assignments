...
public class NormalAccelerationRule extends Rule {

	public NormalAccelerationRule(IInference inferenceMethod) {
	...
	public IFuzzySet infereRuleFromValues(int L, int D, int LK, int DK, int V, int S) {
		...
		antecedentValues.add(Variables.TO_SLOW_VARIABLE.getValueAt(DomainElement.of(V)));
		...
		for (DomainElement element : Domains.ACCELERATION_DOMAIN) {
			set.set(element, inferenceMethod.calculateImplication(antecedentValues,
					Variables.NORMAL_ACCELERATION_VARIABLE.getValueAt(element)));
		}
		return set;
	}
	...
	public String getName() {
		return "Normal acceleration rule";
	}
